...
setsid bash -c 'cd /opt/paperclip && pnpm paperclipai onboard --yes' >"$PAPERCLIP_ONBOARD_LOG" 2>&1 &
...
pnpm paperclipai auth bootstrap-ceo >"$PAPERCLIP_BOOTSTRAP_LOG" 2>&1
PAPERCLIP_INVITE_URL=$(grep -m1 "Invite URL:" "$PAPERCLIP_BOOTSTRAP_LOG" | sed 's/.*Invite URL: //')
PAPERCLIP_INVITE_EXPIRY=$(grep -m1 "Expires:" "$PAPERCLIP_BOOTSTRAP_LOG" | sed 's/.*Expires: //')
rm -f "$PAPERCLIP_BOOTSTRAP_LOG"
if [[ -z "$PAPERCLIP_INVITE_URL" ]]; then
  msg_error "Failed to generate Paperclip admin invite"
  exit 1
fi
