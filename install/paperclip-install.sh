...
pnpm paperclipai onboard --yes >"$PAPERCLIP_ONBOARD_LOG" 2>&1 &
PAPERCLIP_ONBOARD_PID=$!
for _ in {1..60}; do
  if [[ -f /opt/paperclip-data/instances/default/config.json ]] && grep -q "Invite URL:" "$PAPERCLIP_ONBOARD_LOG"; then
    break
  fi
...
PAPERCLIP_INVITE_URL=$(grep -m1 "Invite URL:" "$PAPERCLIP_ONBOARD_LOG" | sed 's/.*Invite URL: //')
PAPERCLIP_INVITE_EXPIRY=$(grep -m1 "Expires:" "$PAPERCLIP_ONBOARD_LOG" | sed 's/.*Expires: //')
rm -f "$PAPERCLIP_ONBOARD_LOG"
if [[ ! -f /opt/paperclip-data/instances/default/config.json ]] || [[ -z "$PAPERCLIP_INVITE_URL" ]]; then
